...
public class Utils {
	...
	public static Beacon beaconFromLeScan(BluetoothDevice device, int rssi, byte[] scanRecord) {
		...
		for (int i = 0; i < scanRecord.length; i++) {
			...
			if (unsignedByteToInt(scanRecord[(i + 1)]) != MANUFACTURER_SPECIFIC_DATA) {
				i += payloadLength;
			} else {
				if (payloadLength == 26) {
					if ((unsignedByteToInt(scanRecord[(i + 2)]) == 76) && (unsignedByteToInt(scanRecord[(i + 3)]) == 0)
						...
						String proximityUUID = String.format("%s-%s-%s-%s-%s",
								new Object[] { scanRecordAsHex.substring(18, 26), scanRecordAsHex.substring(26, 30),
										scanRecordAsHex.substring(30, 34), scanRecordAsHex.substring(34, 38),
						...
						return new Beacon(proximityUUID, device.getName(), device.getAddress(), major, minor,
								measuredPower, rssi);
					}
	...
	public static String normalizeProximityUUID(String proximityUUID) {
		...
		Preconditions
				.checkArgument(withoutDashes.length() == 32, "Proximity UUID must be 32 characters without dashes");

		return String.format(
				"%s-%s-%s-%s-%s",
				new Object[] { withoutDashes.substring(0, 8), withoutDashes.substring(8, 12),
	...
	public static boolean isBeaconInRegion(Beacon beacon, Region region) {
		...
		return ((region.getProximityUUID() == null) || (beacon.getProximityUUID().equals(region.getProximityUUID())))
	...
	public static double computeAccuracy(Beacon beacon) {
		...
		return calculateAccuracy(beacon.getMeasuredPower(), beacon.getRssi());

		// estimote SDK自有算法,算法有误
//		if (beacon.getRssi() == 0) {
//			return -1.0D;
//		}
//
