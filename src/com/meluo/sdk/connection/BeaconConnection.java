...
import com.meluo.sdk.internal.HashCode;
import com.meluo.sdk.internal.Objects;
import com.meluo.sdk.internal.UnsignedInteger;
import com.meluo.sdk.utils.AuthMath;
import com.meluo.sdk.utils.L;

public class BeaconConnection {

	public static Set<Integer> ALLOWED_POWER_LEVELS = Collections.unmodifiableSet(new HashSet<Integer>(Arrays
			.asList(new Integer[] { -30, -20, -16, -12, -8, -4, 0, 4 })));
	...
	public static class BeaconCharacteristics {
		...
		public String toString() {
			...
			return Objects.toStringHelper(this).add("batteryPercent", this.batteryPercent)
					.add("broadcastingPower", this.broadcastingPower)
					.add("advertisingIntervalMillis", this.advertisingIntervalMillis)
					.add("softwareVersion", this.softwareVersion).add("hardwareVersion", this.hardwareVersion)
					.toString();
		}
	...
	public void writeProximityUuid(String proximityUuid, WriteCallback writeCallback) {
		...
		BluetoothGattCharacteristic uuidChar = this.estimoteService.beforeCharacteristicWrite(
				EstimoteUuid.UUID_NORMAL_CHAR, writeCallback);
	...
	public void writeAdvertisingInterval(int intervalMillis, WriteCallback writeCallback) {
		...
		BluetoothGattCharacteristic intervalChar = this.estimoteService.beforeCharacteristicWrite(
				EstimoteUuid.ADVERTISING_INTERVAL_CHAR, writeCallback);
	...
	public void writeBroadcastingPower(int powerDBM, WriteCallback writeCallback) {
		...
		BluetoothGattCharacteristic powerChar = this.estimoteService.beforeCharacteristicWrite(EstimoteUuid.POWER_CHAR,
				writeCallback);
	...
	public void writeMajor(int major, WriteCallback writeCallback) {
		...
		BluetoothGattCharacteristic majorChar = this.estimoteService.beforeCharacteristicWrite(EstimoteUuid.MAJOR_CHAR,
				writeCallback);
	...
	public void writeMinor(int minor, WriteCallback writeCallback) {
		...
		BluetoothGattCharacteristic minorChar = this.estimoteService.beforeCharacteristicWrite(EstimoteUuid.MINOR_CHAR,
				writeCallback);
	...
	private final BluetoothGattCallback bluetoothGattCallback = new BluetoothGattCallback() {
		...
		public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
			...
			if (status == 0) {
				((BluetoothService) BeaconConnection.this.uuidToService.get(characteristic.getService().getUuid()))
