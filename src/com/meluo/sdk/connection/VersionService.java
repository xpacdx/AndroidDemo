...
public class VersionService implements BluetoothService {
	...
	public void processGattServices(List<BluetoothGattService> services) {
		...
		for (BluetoothGattService service : services) {
			if (EstimoteUuid.VERSION_SERVICE.equals(service.getUuid())) {
				this.characteristics.put(EstimoteUuid.HARDWARE_VERSION_CHAR,
						service.getCharacteristic(EstimoteUuid.HARDWARE_VERSION_CHAR));

				this.characteristics.put(EstimoteUuid.SOFTWARE_VERSION_CHAR,
						service.getCharacteristic(EstimoteUuid.SOFTWARE_VERSION_CHAR));
			}
		}
	}
	...
	public String getSoftwareVersion() {
		...
		return this.characteristics.containsKey(EstimoteUuid.SOFTWARE_VERSION_CHAR) ? getStringValue(((BluetoothGattCharacteristic) this.characteristics
				.get(EstimoteUuid.SOFTWARE_VERSION_CHAR)).getValue()) : null;
	}

	public String getHardwareVersion() {

		return this.characteristics.containsKey(EstimoteUuid.HARDWARE_VERSION_CHAR) ? getStringValue(((BluetoothGattCharacteristic) this.characteristics
				.get(EstimoteUuid.HARDWARE_VERSION_CHAR)).getValue()) : null;
	}
