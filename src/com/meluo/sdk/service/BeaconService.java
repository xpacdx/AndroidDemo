...
import com.meluo.sdk.utils.EstimoteBeacons;
...
public class BeaconService extends Service {
	...
	public void onCreate() {
		...
		this.afterScanBroadcastPendingIntent = PendingIntent.getBroadcast(getApplicationContext(), 0,
				AFTER_SCAN_INTENT, 0);
		this.scanStartBroadcastPendingIntent = PendingIntent.getBroadcast(getApplicationContext(), 0,
				SCAN_START_INTENT, 0);
	}
	...
	//	this.alarmManager.set(2, SystemClock.elapsedRealtime() + delayMillis, pendingIntent);
		this.alarmManager.set(2, delayMillis, pendingIntent);
	}
	...
	private class AfterScanCycleTask implements Runnable {
		...
		private List<MonitoringRegion> findEnteredRegions(long currentTimeMillis) {
			...
			for (Beacon beacon : BeaconService.this.beaconsFoundInScanCycle.keySet()) {
				for (MonitoringRegion monitoringRegion : matchingMonitoredRegions(beacon)) {
		...
		public void run() {
			...
			if (BeaconService.this.scanWaitTimeMillis() == 0L) {
				...
				BeaconService.this.setAlarm(BeaconService.this.scanStartBroadcastPendingIntent, EXPIRATION_MILLIS);
			}
		}
	}
	...
	private void checkNotOnUiThread() {
		...
		Preconditions.checkNotNull(Boolean.valueOf(this.handlerThread.getLooper() == Looper.myLooper()),
				...
				if (state == 10) {
					...
					BeaconService.this.handler.post(new Runnable() {
						...
						public void run() {
							...
							if ((!BeaconService.this.monitoredRegions.isEmpty())
									|| (!BeaconService.this.rangedRegions.isEmpty())) {
								L.i(String.format("Bluetooth is ON: resuming scanning (monitoring: %d ranging:%d)",
										new Object[] { Integer.valueOf(BeaconService.this.monitoredRegions.size()),
												Integer.valueOf(BeaconService.this.rangedRegions.size()) }));
	...
	private BroadcastReceiver afterScanBroadcastReceiver = new BroadcastReceiver() {
	...
	private BroadcastReceiver scanStartBroadcastReceiver = new BroadcastReceiver() {
